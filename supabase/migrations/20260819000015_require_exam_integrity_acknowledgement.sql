...
BEGIN
  ...
  SELECT COUNT(*) INTO v_blueprint_count FROM public.exam_blueprint_config WHERE quiz_id = p_quiz_id AND is_active = TRUE;
  IF v_blueprint_count > 0 THEN
    FOR v_blueprint IN SELECT * FROM public.exam_blueprint_config WHERE quiz_id = p_quiz_id AND is_active = TRUE ORDER BY display_order LOOP
      FOR v_question_id IN
        SELECT q.id FROM public.certification_question_bank q
        WHERE q.certification_type = v_quiz.certification_type AND q.language = v_quiz.language AND q.is_active = TRUE
          AND ((v_blueprint.competency_id IS NULL) OR q.competency_id = v_blueprint.competency_id)
          AND ((v_blueprint.competency_section IS NULL) OR q.competency_section = v_blueprint.competency_section)
        ORDER BY random() LIMIT v_blueprint.question_count
      LOOP
        v_order_counter := v_order_counter + 1;
        INSERT INTO public.exam_attempt_questions (attempt_id, question_id, display_order) VALUES (v_attempt.id, v_question_id, v_order_counter);
      END LOOP;
    END LOOP;
  ELSE
    FOR v_question_id IN
