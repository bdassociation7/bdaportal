-- Bind all candidate-sensitive certification actions to one active secure session token.
DROP FUNCTION IF EXISTS public.require_active_secure_exam_session(UUID);
DROP FUNCTION IF EXISTS public.get_certification_attempt_questions(UUID);
DROP FUNCTION IF EXISTS public.get_certification_attempt_answers(UUID);
DROP FUNCTION IF EXISTS public.save_exam_answer(UUID, UUID, UUID[], INTEGER);
DROP FUNCTION IF EXISTS public.finalize_certification_exam_attempt(UUID, BOOLEAN);
...
REVOKE ALL ON FUNCTION public.get_certification_attempt_questions(UUID, UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_certification_attempt_answers(UUID, UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.save_exam_answer(UUID, UUID, UUID[], UUID, INTEGER) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.finalize_certification_exam_attempt(UUID, UUID, BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.require_active_secure_exam_session(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_certification_attempt_questions(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_certification_attempt_answers(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.save_exam_answer(UUID, UUID, UUID[], UUID, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.finalize_certification_exam_attempt(UUID, UUID, BOOLEAN) TO authenticated;
