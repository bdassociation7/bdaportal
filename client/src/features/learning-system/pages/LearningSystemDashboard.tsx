...
function CompetencyCard({
  ...
  const progress =
    (module.user_progress as { progress_percentage?: number } | undefined)
    ...
    >
        ...
        <img
          src={
            competencyImages[
              Math.max(
                0,
                ((module.order_index ?? 1) - 1) % competencyImages.length,
              )
            ]
          }
          alt=""
      ...
      <div className="p-5">
        <div className="mb-4 flex items-start justify-between gap-4">
          ...
          <span
            className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] ${
              isComplete
                ? "bg-[#e6f4ff] text-[#0f91e0]"
                : "bg-[#f6f9fe] text-slate-400"
            }`}
          >
            {isComplete
              ? "Completed"
        ...
        <p className="mt-2 line-clamp-2 min-h-[38px] text-xs leading-relaxed text-slate-500">
          {module.description ||
            "Build practical capability through focused BDA learning content."}
        </p>
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#0f91e0]">
            {eyebrow}
          </p>
          <h2 className="mt-1 text-2xl font-bold tracking-[-0.02em] text-[#0d1f4e]">
            {title}
          </h2>
...
const resources = [
  {
    title: "Mock Exams",
    description:
      "Practise under timed conditions and build confidence before your certification exam.",
  ...
  },
  {
    ...
    description:
      "Follow a structured preparation plan designed for steady, focused progress.",
  ...
  },
  {
    title: "BDA Glossary",
    description:
      "Review essential business development terminology and definitions.",
  ...
  },
  {
    title: "BDA BoCK",
    description:
      "Access the official BDA Body of Competency and Knowledge reference.",
  ...
  },
  {
    ...
    description:
      "Explore membership benefits and continue your professional development journey.",
  ...
  return (
    <section className="border-t border-[#e2eaf6] pt-10">
      <div className="mb-5">
        ...
        <p className="mt-1.5 max-w-2xl text-sm leading-relaxed text-slate-500">
          Extend your learning with BDA resources designed to support
          certification readiness and ongoing professional growth.
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-5">
                <h3 className="text-base font-bold leading-snug text-[#0d1f4e]">
                  {resource.title}
                </h3>
...
export function LearningSystemDashboard() {
  ...
  const basePath = location.pathname.startsWith("/ecp/")
    ...
    : location.pathname.startsWith("/instructor/")
      ...
      : "/learning-system";
  const portalPath = location.pathname.startsWith("/ecp/")
    ? "/ecp/dashboard"
    : location.pathname.startsWith("/instructor/")
      ? "/instructor/dashboard"
      : "/individual/dashboard";

  const { data: accessSummary, isLoading: isLoadingAccess } = useUserAccesses(
    user?.id,
  );
  const hasAccess = accessSummary?.has_en;
  const displayAccess =
    accessSummary?.accesses?.find((access) => access.language === "EN") ||
  ...
  const { data: modules = [], isLoading: isLoadingModules } =
    useModulesWithProgress(user?.id, "CP", Boolean(hasAccess), "en");

  const behaviouralModules = modules.filter(
    (module) => module.section_type === "behavioral",
  );
  const knowledgeModules = modules.filter(
    (module) => module.section_type === "knowledge_based",
  );
    ...
    return (
      <div className="flex min-h-[calc(100vh-82px)] items-center justify-center bg-[#f8f9fb] px-4 sm:min-h-[calc(100vh-92px)]">
        <div className="w-full max-w-md rounded-3xl border border-[#dbeafe] bg-white p-10 text-center shadow-xl">
          ...
          <h2 className="mb-2 text-xl font-bold text-[#0d1f4e]">
            Access Required
          </h2>
          <p className="mb-7 text-sm leading-relaxed text-slate-500">
            You need to purchase the BDA Learning System to access learning
            content and professional development tools.
          </p>
          <button
            type="button"
            onClick={() =>
              window.open(
                "https://bda-global.org/en/store/bda-learning-system/",
                "_blank",
              )
            }
  ...
  return (
    <div className="flex min-h-[calc(100vh-82px)] flex-col bg-white sm:min-h-[calc(100vh-92px)]">
        ...
        <div className="relative mx-auto flex max-w-[1640px] flex-col gap-8 px-6 sm:px-10 lg:flex-row lg:items-end lg:justify-between lg:px-16 xl:px-24">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-white/80 sm:text-base">
              Follow a structured path through the BDA Body of Competency and
              Knowledge, strengthen your professional practice, and prepare with
              confidence.
            </p>
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-[#1c4a8b] shadow-[0_7px_18px_rgba(0,0,0,0.18)] transition-colors hover:bg-[#f0f6ff]"
              >
      ...
      <main className="mx-auto w-full max-w-[1640px] space-y-20 px-6 py-14 sm:px-10 sm:py-16 lg:px-16 lg:py-20 xl:px-24">
        <section>
          <div className="mb-5">
            ...
            <p className="mt-1.5 max-w-2xl text-sm leading-relaxed text-slate-500">
              Choose a target exam window and let your BDA learning journey work
              around your professional goal.
            </p>
          </div>
      ...
      <footer className="mt-auto w-full bg-gradient-to-r from-[#0d1f4e] via-[#1c4a8b] to-[#0f91e0] px-6 py-5 text-center text-sm font-medium text-white/90 sm:px-10 lg:px-16">
        © {new Date().getFullYear()} Business Development Association. All
        rights reserved.
      </footer>
    </div>
  );
